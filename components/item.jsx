...
export default function ItemDao(props){
    ...
    useEffect(()=>{
        if(!item.body)return;
        let objFormat = PublicJs.ToJSON(item.body);
        setObj(objFormat)
    },[item])
    ...
    return <Link href={`/dao/${formatStr('slug')}`}><div className="!no-underline" >
            <div className="hidden sm:block rounded-2xl border-2 border-gray-100 overflow-hidden shadow-md bg-white hover:shadow-xl hover:-translate-y-1 transition-all ease duration-200">
                <SpanBoxli>
                    <span />
                    <img alt="Developer DAO" src={formatStr('cover')}  className="duration-700 ease-in-out grayscale-0 blur-0 scale-100" />
                </SpanBoxli>
                <div className="py-6 px-5 h-36">
                    ...
                    <p className="!mt-3 !text-gray-800 italic !text-md !leading-snug !font-normal">{formatStr('tagline')} –  {formatStr('symbol')}</p></div>
            </div>
            <div className="sm:hidden overflow-hidden rounded-xl flex items-center md:h-48 h-36 border-2 border-gray-100 focus:border-black active:border-black bg-white transition-all ease duration-200">
                ...
                <div className="py-6 px-5 w-3/5 !relative">
                    ...
                    <p className="!mt-3 !text-gray-800 italic !text-sm sm:!text-base !leading-snug !font-normal line-clamp-3">Accelerating the Education and Impact of a new wave of Web3 Builders. – {formatStr('symbol')}</p></div>
            </div>
    </div></Link>
}